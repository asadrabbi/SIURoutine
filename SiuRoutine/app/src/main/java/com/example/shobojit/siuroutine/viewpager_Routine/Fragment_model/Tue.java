...
public class Tue extends Fragment {
    ...
    String dptName;
    Context cn;
    List<String> TueClass,TueRoom,TueTeacher,TueStart,TueEnd;
    String Data;
    @Override
    ...
    void Intialized(View v){
        TueList = (ListView) v.findViewById(R.id.tuelist);
        TueList.setAdapter(new TueAdapter(getActivity(),TueClass,TueRoom,TueTeacher,TueStart,TueEnd));
    }

    void getDptName(){
        TueClass = new ArrayList<>();
        TueRoom = new ArrayList<>();
        TueTeacher = new ArrayList<>();
        TueStart = new ArrayList<>();
        TueEnd = new ArrayList<>();
    ...
    void getJsonData(){
        try {
            ...
            JSONArray js2 = js1.getJSONArray("TUE");
            for (int i = 0;i<js2.length();i++){
                TueClass.add(js2.getJSONObject(i).getString("name"));
                TueRoom.add(js2.getJSONObject(i).getString("room"));
                TueTeacher.add(js2.getJSONObject(i).getString("tname"));
                TueStart.add(js2.getJSONObject(i).getString("start"));
                TueEnd.add(js2.getJSONObject(i).getString("end"));
            }
...
class  TueAdapter extends ArrayAdapter {
    private Activity cn;
    List <String> clss,room,teacher,start,end;
    public TueAdapter( Context context, List<String> clss,List<String> room,List<String> teacher,List<String> start,List<String> end) {
